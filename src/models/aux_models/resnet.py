...
# ------------------------------------------------------------------------------
class ResNetAuxiliaryFull(aux_models.GradScalarAuxiliaryModel):
    def __init__(
        self,
        server,
        block: Type[Union[BasicBlock, Bottleneck]],
        ...
        super(ResNetAuxiliaryFull, self).__init__(
            server, device, align_epochs, align_batch_size, max_dataset_size
        )
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
# ------------------------------------------------------------------------------
class ResNetAuxiliaryLinear(aux_models.GradScalarAuxiliaryModel):
    def __init__(
        self,
        server,
        block: Type[Union[BasicBlock, Bottleneck]],
        ...
        super(ResNetAuxiliaryLinear, self).__init__(
            server, device, align_epochs, align_batch_size, max_dataset_size
        )
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # Only keep the linear layer forward pass 
        x = self.avgpool(x) # TODO: Remove avgpool?
        x = torch.flatten(x, 1)
        x = self.fc(x)
        return x
...
# ------------------------------------------------------------------------------
def _resnet_sl_auxiliary_linear(
    server,
    block: Type[Union[BasicBlock, Bottleneck]],
    layers: List[int],
    in_planes: int,
    ...
    align_batch_size=100, max_dataset_size=1000,
    **kwargs: Any,
) -> List[ResNetAuxiliaryLinear]:

    aux_model = ResNetAuxiliaryLinear(
        server, block, layers, in_planes, device=device,
        align_epochs=align_epochs, align_step=align_step,
        align_batch_size=align_batch_size, max_dataset_size=max_dataset_size,
        **kwargs
...
# ------------------------------------------------------------------------------
def _resnet_sl_auxiliary_full_model(
    server,
    block: Type[Union[BasicBlock, Bottleneck]],
    layers: List[int],
    in_planes: int,
    ...
    align_batch_size=100, max_dataset_size=1000,
    **kwargs: Any,
) -> List[ResNetAuxiliaryFull]:

    aux_model = ResNetAuxiliaryFull(
        server, block, layers, in_planes, device=device,
        align_epochs=align_epochs, align_step=align_step,
        align_batch_size=align_batch_size, max_dataset_size=max_dataset_size,
        **kwargs
...
def resnet18_sl_aux_linear(server, layers=None, in_planes: int = 128,
    ...
    .. autoclass:: torchvision.models.ResNet18_Weights
        :members:
    """
    layers = [2, 2, 0, 0] # Should be valid, since no other layers besides linear should be used
    return _resnet_sl_auxiliary_linear(
        server, BasicBlock, layers, in_planes, weights, progress,
        num_classes=num_classes, device=device, **kwargs
    )
...
def resnet18_sl_full(server, layers=None, in_planes: int = 128,
    ...
    """
    if layers is None: layers = [2, 2, 2, 2]
    return _resnet_sl_auxiliary_full_model(
        server, BasicBlock, layers, in_planes, weights, progress,
        num_classes=num_classes, device=device, **kwargs
    )
